...
export class RagService {
    ...
    async processAndUpsert(userId: number, file: Express.Multer.File, namespace?: string) {
        ...
        if (!['pdf', 'docx'].includes(ext)) throw new BadRequestException('Only PDF and DOCX files are supported.');
    ...
    async extractText(file: Express.Multer.File, type: 'pdf' | 'docx'): Promise<string> {
        if (!file || !file.buffer) {
        ...
        try {
          if (type === 'pdf') {
            // pdf-parse handles all pages internally
    ...
    async generateRAGResponse(
        ...
        console.log('Generating RAG response for user', userId, 'with query', query);
        if (!query || query.trim().length === 0) {
            throw new BadRequestException('Query cannot be empty');
        }
        ...
        try {
            ...
            const chatModel = this.config.get<string>('CHAT_MODEL') || 'gpt-4o-mini';
